...
async function handleTier2(r, text, fromVoice) {
  ...
  html += `<br><button class="btn ghost small" data-req-skill="examinar" data-req-prompt="resume el tema: ${esc(text)}">Pedir a Claude Code que lo resuma a fondo</button>${tierNote(r)}`;
...
async function handleTier1(text, intent, fromVoice) {
  ...
  if (results.length) {
    ...
    else html += `<br><button class="btn ghost small" data-req-skill="examinar" data-req-prompt="resume el tema: ${esc(text)} (parte de la nota «${esc(top.note.title)}»)">Pedir a Claude Code que lo resuma a fondo</button>`;
  ...
  if (intent.fallbackAI) {
    const btn = state.settings.apiKey
      ...
      : `<button class="btn ghost small" data-req-skill="examinar" data-req-prompt="resume el tema: ${esc(text)}">Que Claude Code lo resuma a fondo</button>`;
  ...
  addMsg('brainer', `No tengo nada guardado sobre “${esc(intent.query)}”. Puedo guardarlo como nota o pedir a Claude Code que lo investigue. <button class="btn ghost small" data-create="${esc(intent.query)}">Guardar como nota</button> <button class="btn ghost small" data-req-skill="examinar" data-req-prompt="resume el tema: ${esc(text)}">Que Claude Code lo resuma a fondo</button>`, { speakText: 'No tengo nada guardado sobre eso. ¿Lo guardo o lo investigo?' });
}
...
async function runSkill(id, { prompt } = {}) {
  ...
  if (!p && id === 'escribir') { p = window.prompt('¿Qué brief escribimos? (título o tema)'); if (!p) return; }
...
function bind() {
  ...
  $('#note-ai').onclick = async () => {
    ...
    if (!state.settings.apiKey) {
      const p = window.prompt('¿Qué quieres que haga Claude Code con esta nota?', `resume el tema: «${n.title}» con preguntas de repaso`);
      if (!p) return;
      $('#note-panel').hidden = true; await runSkill('examinar', { prompt: p }); return;
    }
